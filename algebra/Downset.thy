...
class preorder_bot = preorder +
  fixes bottom :: "'a \<Rightarrow> 'a" ("_\<^sub>\<bottom>" [1000] 999)
  assumes bot_least: "\<And>x y. x \<le> y \<Longrightarrow> bottom y \<le> x"
...
context preorder_bot begin

definition "\<Omega> = \<Down>(range bottom)"

lemma in_omega[simp]: "x \<in> \<Omega> \<longleftrightarrow> (\<exists>y. x \<le> bottom y )"
  apply (safe; clarsimp simp: \<Omega>_def image_iff downset_set_def )
  by blast


end

typedef (overloaded) ('a) downset = "{S. \<Down>S = (S :: ('a :: preorder_bot set)) \<and> \<Omega> \<subseteq> S} " 
...
lemma Din_intro': "a \<in> \<Omega> \<Longrightarrow>  x \<le> a  \<Longrightarrow> x \<down>\<in> S"
  apply (clarsimp simp: downset_def downset_set_def)
  using Din.rep_eq Rep_downset in_omega mem_Collect_eq order.partial_preordering_axioms partial_preordering.trans by fastforce


lemma bot_in[simp]: "bottom x \<down>\<in> S"
  apply (rule Din_intro'[where a="bottom x"]; clarsimp)
  apply (blast)
  done
...
lemma downclosed_inverseI: "\<Down>y = y \<Longrightarrow> \<Omega> \<subseteq> y \<Longrightarrow> x \<in> y \<Longrightarrow>  x \<down>\<in> (Abs_downset y) "
  apply (clarsimp simp: Din_def)
  apply (subst  Abs_downset_inverse)
   apply (clarsimp)
  using Downset.preorder_bot_class.bot_least downset_set_def by blast
...
lemma [simp]:"\<Down>(\<Down>X) = \<Down>X"
  apply (safe; clarsimp simp: downset_set_def)
  using order_trans apply metis
  apply (blast)
  done

instantiation downset :: (preorder_bot) complete_lattice begin 
lift_definition top_downset :: "'a downset" is " (\<Down>UNIV) " by (clarsimp)
lift_definition bot_downset :: "'a downset" is " \<Down>(\<Omega>) " by (fastforce simp: \<Omega>_def)

lift_definition Sup_downset :: "'a downset set \<Rightarrow> 'a downset" is "\<lambda>S. \<Union>S \<union> \<Down>(\<Omega>)"
  apply (clarsimp simp: downset_set_def downset_def eta_def)
...
sublocale preorder_bot _ _ "\<lambda>_. \<bottom>" by (standard; clarsimp simp: bot_least)
end *)
...
instance
  ...
  apply (clarsimp)
   apply (rule_tac x="(\<lambda>Y. \<Down>(SOME X.  X \<in> Y \<and> x \<in> X ) \<union> \<Down>\<Omega>) ` A " in exI)
  apply (intro conjI)
    apply (rule_tac x="(\<lambda>Y. \<Down>(SOME X.  X \<in> Y \<and> x \<in> X ) \<union> \<Down>\<Omega>)" in exI, clarsimp simp: down_union_distrib) 


    apply (subst Un_commute)
    apply (erule_tac x=Y in ballE)
     apply (metis (mono_tags, lifting) \<Omega>_def down_down someI_ex sup.absorb2)
  apply (clarsimp)
   apply (simp add: someI2_bex, clarify)
   apply (simp add: down_union_distrib)
  apply (clarify)
...
lemma (in preorder_bot) [simp]: "bottom x \<in> \<down> x"
...
lemma in_down_bot_trans[simp]: "(x :: 'a :: preorder_bot) \<in> \<down>(bottom y) \<Longrightarrow> x \<in> \<down>y"
  apply (clarsimp simp: in_down_iff bot_least)
  using bot_least order_trans 
  by blast
