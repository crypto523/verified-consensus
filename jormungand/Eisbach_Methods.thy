...
notepad begin
 fix D C
  ...
  apply (only_asm \<open>simp\<close>) 
  apply (only_concl \<open>simp add: DC\<close>)
  by (rule DC)
...
notepad begin
  ...
  have "A \<and> A" "A \<and> A" "B"
    apply (find_goal \<open>fails \<open>simp\<close>\<close>)
    apply (rule B)
    by (simp add: A)+

  have  "B" "A" "A \<and> A"
    apply (find_goal \<open>succeeds \<open>simp\<close>\<close>) 
    apply (rule conjI)
    by (rule A B)+
